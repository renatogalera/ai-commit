...
8. **Interactive Commit Splitting (optional)**  
   If `--interactive-split` is used, AI-Commit shows a separate TUI that breaks the diff into chunks or “hunks,” allowing you to pick which lines go into each commit. The partial commit message is then generated by AI for each selected set of changes.
...
3. Use **Space** to toggle which chunks you want to include in a partial commit.  
4. Press **C** to commit the selected chunks. The commit message is then generated by AI.  
